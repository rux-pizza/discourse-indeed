...
function willQuote(e) {
  ...
  return $target.hasClass('indeed-button') || $target.closest('.empty-reply').length;
}
...
export default Ember.Component.extend({
  ...
  _selectionChanged() {
    ...
    Ember.run.scheduleOnce("afterRender", () => {
      ...
      if (showAtEnd) {
        top = top + 20 - $indeedButton.outerHeight() - 5;
        left = Math.min(left + 10, $(window).width() - indeedButton.outerWidth());
      } else {
        top = top - 2 * indeedButton.outerHeight() - 5 - 5;
      }

      indeedButton.offset({ top, left });
    });
  ...
  didInsertElement() {
    ...
    $(document).on("mousedown.indeed-button", e => {
      this._isMouseDown = true;
      this._reselected = false;
      if (!willQuote(e)) {
        this._hideButton();
      }
    }).on("mouseup.indeed-button", () => {
      this._isMouseDown = false;
      onSelectionChanged();
    }).on("selectionchange.indeed-button", () => {
  ...
  willDestroyElement() {
    $(document).off("mousedown.indeed-button")
      .off("mouseup.indeed-button")
      .off("selectionchange.indeed-button");
  },
